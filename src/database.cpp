...
void database_parse(const char **filenames, int nproteins)
{
	...
	database.naminoacids = (int*)smalloc(sizeof(int)*nproteins); /* Number of amino acids of each protein */

	/** Atoms initialization */
	database.maxatoms = 0;
	database.minatoms = INT_MAX;
	database.natoms =  (int *)smalloc(sizeof(int)*nproteins);   /* Number of atoms of each protein */
	...
	for (int wprotein = 0; wprotein < nproteins; wprotein++)
	{
		...
		/* Get number of aminoacids and atoms. */
...
void database_read (const char **filenames, int nproteins)
{
	...
	/* Allocate database. */
	database.data_atoms = new struct Atom**[nproteins];
	
	for (int i = 0; i < nproteins; i++)
		database.data_atoms[i] = new struct Atom*[database.maxatoms];
	...
	/* Allocate database. */
	database.data_aminoacids = new struct Aminoacid**[nproteins];
	
	for (int i = 0; i < nproteins; i++)
		database.data_aminoacids[i] = new struct Aminoacid*[database.maxaminoacids];	
